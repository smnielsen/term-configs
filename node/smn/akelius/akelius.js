...
const login = async (page, { username, password }) => {
  const url = 'https://rent.akelius.com/en_UK/search/germany/apartment/berlin';
  ...
  await Promise.all([page.waitForNavigation({ waitUntil: 'networkidle0', timeout: 5000 }), page.click('#wp-submit')]);

  log('Should be logged in', page.url());

  // Let's get the auth cookie
  const cookies = await page.cookies();
  const authCookie = cookies.reduce((str, cookie) => `${str}; ${cookie.name}=${cookie.value}`, '');
